...
class JournalTypeController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            if ($request->table == 'journal_type') {
                $data = JournalType::latest();
                return $this->generateDataTable($data);
            }
        }

        return view('backend.layouts.journal.journal-type.index');
    }
        ...
        return DataTables::of($query)
            ...
            ->addColumn('action', function ($row) {
                return '<div class="d-flex gap-1">
                            <a href="' . route('journal-type.edit', $row->id) . '" class="btn btn-sm btn-primary">
    ...
    public function edit(string $id)
    {
        ...
        return view('backend.layouts.journal.journal-type.edit', compact('journalType'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $journalType = JournalType::findOrFail($id);
        $journalType->title = $request->title;
        $journalType->description = $request->description;

        if ($request->hasFile('icon')) {
            $file = $request->file('icon');
            $fileName = time() . '_' . $file->getClientOriginalName();
            
            $file->move(public_path('uploads/journal_types'), $fileName);
            
            $journalType->icon = $fileName;
        }

        $journalType->save();

        return redirect()->route('journal.index')->with('success', 'Journal Type updated!');
    }
    ...
    {
        $user = JournalType::findOrFail($id);
